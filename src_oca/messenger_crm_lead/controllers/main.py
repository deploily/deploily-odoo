...
class MessengerWebhook(http.Controller):
    ...
    def verify_webhook(self, **kwargs):
        ...
        verify_token = ICP.get_param('messenger_crm_lead.verify_token', 'odoo_messenger_123')
        ...
        _logger.info("Webhook verify: mode=%s token=%s", hub_mode, hub_verify_token)

        if hub_mode == 'subscribe' and hub_verify_token == verify_token:
            return request.make_response(hub_challenge or '', [('Content-Type', 'text/plain')])
    ...
    def receive_message(self, **kwargs):
        try:
            print("########################################################Received POST webhook with data:")
            raw_data = request.httprequest.data
            data = json.loads(raw_data)

            _logger.info("📩 RAW WEBHOOK RECEIVED: %s", raw_data)
            _logger.info("📩 PARSED JSON: %s", data)

            obj = data.get('object')
            _logger.info("📦 OBJECT TYPE: %s", obj)

            if obj == 'page':
                entries = data.get('entry', [])
                _logger.info("📥 ENTRIES COUNT: %s", len(entries))

                self._process_entries(entries, source='messenger')

            elif obj == 'instagram':
                entries = data.get('entry', [])
                _logger.info("📥 IG ENTRIES COUNT: %s", len(entries))

                self._process_entries(entries, source='instagram')

            else:
                _logger.warning("⚠️ Unknown object type: %s", obj)
