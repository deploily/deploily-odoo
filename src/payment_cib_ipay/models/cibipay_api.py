...
class CibIPayApi():
    ...
    def get_cibipay_urls(self, state):
        """ CIB IPay URLS:
            - standard order: POST address for form-based """
        environment = 'test' if state == 'test' else 'cib'
        return {
            "cibipay_register_url": "https://%s.satim.dz/payment/rest/register.do" % (environment,),
            "cibipay_confirm_order_url": "https://%s.satim.dz/payment/rest/confirmOrder.do" % (environment,),
            "cibipay_refund_url": "https://%s.satim.dz/payment/rest/refund.do" % (environment,)
        }
    ...
    def sendRegisterOrder(self, order_id, order_total,confirm_url, fail_url, description =""):
        ...
        base_url = self.get_cibipay_urls(self.is_testing_mode)['cibipay_register_url']
    ...
    def get_payment_status(self, satim_order_id) :
        ...
        payment_status['expiration'] = (response['expiration'] if 'expiration' in response.keys() else "")
        payment_status['cardholderName'] = (response['cardholderName'] if 'cardholderName' in response.keys() else "")
        payment_status['orderStatus'] = (response['OrderStatus'] if 'OrderStatus' in response.keys() else "")
        ...
        if response['params']:
            ...
            payment_status['respCode_desc'] = (params['respCode_desc'] if 'respCode_desc' in params.keys() else "")
            payment_status['respCode'] = (params['respCode'] if 'respCode' in params.keys() else "")
            payment_status['params-udf1'] = (params['udf1'] if 'udf1' in params.keys() else "")
            payment_status['params-udf2'] = (params['udf2'] if 'udf2' in params.keys() else "")
            payment_status['params-udf3'] = (params['udf3'] if 'udf2' in params.keys() else "")
            payment_status['params-udf4'] = (params['udf4'] if 'udf4' in params.keys() else "")
            payment_status['params-udf5'] = (params['udf5'] if 'udf5' in params.keys() else "")
                  
        return payment_status

    def SendConfirmOrder(self, order_id) :
        
        base_url = self.get_cibipay_urls(self.is_testing_mode)['cibipay_confirm_order_url']
    ...
    def SendRefundOrder(self, order_id, amount):
        ...
        base_url = self.get_cibipay_urls(self.is_testing_mode)['cibipay_refund_url']
