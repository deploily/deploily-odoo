...
class EdahabiaRefundTransactionWizard(models.TransientModel):
    _name = "payment_edahabia.refund_wizard"
    ...
    def action_do_refund(self):
        ...
        acquirer = (
            self.env["payment.acquirer"].sudo().search([("provider", "=", "edahabia")])
        )
        edahabia = acquirer._get_edahabia_api()

        amount_currency = int(self.amount) * 100
        edahabia_order_id = self.order_id
        result = edahabia.do_refund(edahabia_order_id, amount_currency)

        if result["returnCode"] != 200:
            _logger.info(
                "EDAHABIA server request error ("
                + result["returnCode"]
                + ") : "
                + result["text_error"]
            )
            raise ValidationError(
                "EDAHABIA server request error ({}) : '{}'".format(
        ...
        values = {
            ...
            "edahabia_mdorder": trans.edahabia_mdorder,
            "edahabia_cardholder_name": trans.edahabia_cardholder_name,
            "edahabia_deposit_amount": trans.edahabia_deposit_amount,
            "edahabia_pan": trans.edahabia_pan,
            "payment_id": trans.payment_id.id,
            "edahabia_error_code": error_code,
            "edahabia_error_message": message,
        }
        ...
        if error_code == "0":
            ...
            _logger.info(
                "EDAHABIA refund transaction success ({}) : '{}' ".format(
                    error_code, message
                )
            )
        else:
            _logger.info(
                "EDAHABIA refund transaction error ({}) : '{}' ".format(
                    error_code, message
                )
            )
