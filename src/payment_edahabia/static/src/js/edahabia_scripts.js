...
function updateSubmit() {
    var captcha = this.$("#g-recaptcha-response");
    var checkbox_terms = this.$("#agree");
    var submit_pay = this.$('#o_payment_form_pay')[0];
    
    if (captcha.length != 1 || checkbox_terms.length != 1)
        return false;

    if (!!captcha[0].value && checkbox_terms[0].checked) {
...
odoo.define('payment_edahabia.payment_form', function (require) {
    "use strict";
        ...
        payEvent: function (ev) {
            ...
            if (checked_radio.length === 1) {
                ...
                // if the user is going to pay with a form payment, then
                    ...
                    var $check_terms = this.$el.find('input[name="checkbox"]');
                    var $captcha = this.$("#g-recaptcha-response");
                    
                    // if there's a prepare tx url set
                    if ($tx_url.length === 1) {
                        var $final_url;
                        if (checked_radio.dataset.provider == "cibipay" || checked_radio.dataset.provider == "edahabia")
                            $final_url = '/payment/' + checked_radio.dataset.provider + '/transaction/';
                        else $final_url = $tx_url[0].value;
                        ...
                        return this._rpc({
                            ...
                            params: {
                                ...
                                'final_prepare_tx_url': $tx_url[0].value,
                                'check_terms': $check_terms.length === 1 ? $check_terms[0].checked : true,
                                'captcha': $captcha.length === 1 ? $captcha[0].value : "",
                            },
